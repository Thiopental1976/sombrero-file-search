...
| [`campo/`](campo/) | Field tests on real hardware and networks, with the raw evidence (`--json`, kernel log, window geometry): the Bazzite install (Jul 2026), the TrueNAS NAS over a Tailscale link (Sep 2026) and the `.deb` GUI on a 12-disk server (Sep 2026). |
